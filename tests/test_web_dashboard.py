...
def test_safe_url_accepts_http_and_https():
    assert dashboard.safe_url("https://example.com/news") == "https://example.com/news"
    assert dashboard.safe_url("http://localhost:8080") == "http://localhost:8080"


def test_safe_url_rejects_credentials_and_invalid_scheme():
    with pytest.raises(ValueError):
        dashboard.safe_url("ftp://example.com/file")
    with pytest.raises(ValueError):
        dashboard.safe_url("https://user:secret@example.com/news")
...
def test_sources_round_trip_uses_atomic_json(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "SOURCES_DB", sources_path)
    payload = {"added": [{"url": "https://example.com/news"}], "disabled": []}
    dashboard.save_sources(payload)
    assert dashboard.load_sources() == payload
...
def test_effective_scrape_config_applies_additions_and_disables(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "SCRAPE_CONFIG", base)
    monkeypatch.setattr(dashboard, "SOURCES_DB", tmp_path / "sources.json")
    monkeypatch.setattr(dashboard, "ROOT", tmp_path)
    dashboard.save_sources({
        "added": [{"url": "https://added.example/news", "days_back": 7}],
        "disabled": ["https://base.example/news"],
    })
    result = dashboard.effective_scrape_config()
