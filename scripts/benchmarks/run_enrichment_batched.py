...
Architecture:
  [DB reader] â†’ [batch queue] â†’ [3 batch workers] â†’ [result queue] â†’ [DB writer]
...
Throughput expectation:
  ...
  - Parallel (3W):       0.63 chunks/s  (1.58s/chunk)  â† current
  - Batched (3W, B=5):  ~1.5-2.0 chunks/s (0.5-0.7s/chunk)  â† this module

  The batched approach does 1 forward pass for 5 chunks instead of 5
  separate passes. Even though the combined prompt is longer (5Ã—1500 chars),
  a single forward pass of 7500 tokens is faster than 5 passes of 1500
  because GPU utilization is higher and HTTP roundtrips are 5Ã— fewer.
...
def run_batched_enrichment(
    ...
    # Process them individually (batch_size=1) â€” slower but reliable
    if fallback_chunks:
        print(f"\n{'='*60}")
        ...
        print("\nNo fallback needed â€” all chunks parsed successfully in batched mode.")
