"""Run the fast path pipeline over a directory of artifacts.

Usage:
    python scripts/run_fast_path.py --input data/sample/input --output outputs/experiments/E1
"""
...
def index_lancedb(store_db: Path, lance_path: Path, batch_size: int = 64) -> dict:
    ...
    Only embeds NEW chunks (not already in LanceDB) — idempotent and incremental.
...
def _index_lancedb_incremental(store, lance, embedding, batch_size: int = 64) -> dict:
    ...
    if total_chunks > 0 or skipped > 0:
        try:
            from ipa.lancedb_index import _compute_centroids
...
def main() -> None:
    ...
    # Stage 2: LanceDB vector indexing (BGE-M3) — runs after BM25 so corpus is queryable immediately
    lance_report = None
    ...
    if args.watch > 0 and not args.no_lancedb:
        ...
        while True:
            ...
            # Incremental LanceDB indexing — model already loaded, no reload
