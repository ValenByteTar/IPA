"""Run web scraping + OCR over configured sites.

Usage:
    # Scrape all sites from configs/scrape_sites.yaml
    python scripts/run_web_scrape.py

    # Scrape a single URL with explicit pattern
    python scripts/run_web_scrape.py --url https://developer.nvidia.com/blog \\
        --url-pattern "^/blog/[^/]+/$" \\
        --exclude "/blog/category/" "/blog/tag/" "/blog/recent-posts/"

    # Scrape a single URL with CSS selector
    python scripts/run_web_scrape.py --url https://thehackernews.com/ \\
        --selector "article a[href]"

    # Scrape a single URL with heuristics (no pattern/selector)
    python scripts/run_web_scrape.py --url https://example.com/news --days-back 2

    # Skip OCR (just scrape text + images)
    python scripts/run_web_scrape.py --no-ocr

    # Specify languages for OCR
    python scripts/run_web_scrape.py --ocr-langs en es

    # Use a custom config file
    python scripts/run_web_scrape.py --config configs/scrape_sites.yaml
"""
