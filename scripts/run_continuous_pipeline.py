"""Continuous ingestion pipeline — watches Landing/, processes files as they
appear, runs the full pipeline (parse → chunk → store → BM25 → LanceDB →
enrich), then moves processed files to Archive/.

Flow:
    Landing/  (scraper writes here)
        ↓ (watcher detects new files)
    mime → parse (PyMuPDF with smart OCR) → chunk → store → BM25
        ↓
    embedding → LanceDB
        ↓
...
from ipa.embedding_adapter import EmbeddingAdapter
...
def should_summarize(chunk: DocumentChunk, min_chars: int = 800) -> bool:
    ...
    Short chunks are already concise — summarizing them wastes LLM calls
...
def _is_lock_error(error: object) -> bool:
    ...
    transient — the lock may be released after a brief wait.
    """
...
def enrich_selective(store_db: Path, llm: OllamaAdapter,
    ...
    for chunk_id, doc_id, text, content_hash in rows:
        ...
        enriched += 1
        print(f"    Enriched {chunk_id[:16]}... ({len(text)} → {len(result.enriched_text)} chars)")
...
def move_to_archive(filepath: Path, archive_dir: Path) -> None:
    ...
    3. filepath.unlink() with retries — if it still fails, rename to
    ...
    for attempt in range(5):
        try:
            ...
            # Verify deletion — Windows may silently fail
            if not filepath.exists():
                return
            # File still exists after unlink — retry
            ...
            if attempt < 4:
                ...
                try:
                    ...
                    print(f"    ⚠ Could not delete (handle locked) — renamed to {pending.name}")
                except (PermissionError, OSError):
                    # Even rename failed — the file is locked hard.
                    # Log warning but don't raise; the file is already in Archive.
                    print(f"    ⚠ WARNING: Could not delete or rename {filepath.name} — "
...
def cleanup_pending_delete(landing_dir: Path) -> int:
    ...
    for f in landing_dir.rglob("*.pending_delete"):
        try:
            ...
            pass  # still locked — try again next run
    if cleaned:
...
def main() -> None:
    ...
    if args.lancedb_incremental and not args.no_lancedb:
        ...
        # Don't delete existing — we're doing incremental
        ...
        try:
            ...
            if _backlog:
                ...
                for _i in range(0, len(_backlog), _bs):
                    ...
                    _done = _i + len(_batch)
                    print(f"    [backlog] {_done}/{len(_backlog)} → {lancedb_index._table.count_rows()} rows", flush=True)
    ...
    try:
        while True:
            ...
            if new_files:
                ...
                # slow files (.pdf) last — prevents fast files being
                ...
                for filepath in new_files:
                    ...
                    if result and not result["errors"]:
                        ...
                        print(f"    OK: {result['mime_type']} → {result['chunks_created']} chunks, "
                        ...
                        # IO/permission error (Tantivy lock, Windows file lock) —
                        ...
                        for attempt, wait in enumerate([2, 4, 8], 1):
                            ...
                            if result and not result["errors"]:
                                ...
                                total_processed += 1
                                total_chunks += result["chunks_created"]
                                print(f"    OK (retry {attempt}): {result['mime_type']} → {result['chunks_created']} chunks")
                                move_to_archive(filepath, archive)
                                result = None  # mark as handled
                                break
                        if result is not None:
                            # All retries failed — leave in Landing/ for next cycle
                            err = result["errors"][0] if result and result["errors"] else "unknown"
                            print(f"    ERROR (all retries failed): {err}")
                            print(f"    ⚠ Left in Landing/ for retry next cycle")
                            # Do NOT add to processed_files — will be retried
                    else:
                        err = result["errors"][0] if result and result["errors"] else "unknown"
                        print(f"    ERROR: {err}")
                        print(f"    ⚠ Left in Landing/ for retry next cycle")
                        ...
                        if _doc_chunks:
                            ...
                            if _new:
                                ...
                                for _i in range(0, len(_new), _batch_size):
                                    ...
                                    _vectors, _sparse = lancedb_embedding.embed_texts_hybrid(_texts)
                                    lancedb_index.add_chunks(_chunks, _vectors, sparse_weights=_sparse)
                                print(f"    [LanceDB incremental] +{len(_new)} chunks → {lancedb_index._table.count_rows()} total rows")
                    except Exception as e:
                        print(f"    [LanceDB incremental] ERROR: {e}")
            else:
                # No new files — check idle timeout
                idle = time.monotonic() - last_activity
                if idle > args.idle_timeout:
                    print(f"\n[{time.strftime('%H:%M:%S')}] Idle for {idle:.0f}s — no new files.")
                    break
    ...
    # Close runner (normal exit — all files processed or idle timeout)
    runner.close()
